...
def find_best_players(url: str) -> None:
    ...
    for team, players in all_players.items():
        team_best_points[team] = []
        for player in players:
            player_stats = get_player_stats(player["url"], team)
            if(player_stats != None):
                for stats in player_stats:
                    # Players will be sorted based on points, therefore we add all players points
                    if(stats == "points"):
                        team_best_points[team].append(player_stats[stats])
                    player[stats] = player_stats[stats]
        # Sort all the players in a team through points and ranked from highest to low
        team_best_points[team].sort(reverse=True)
        # The first three players are the ones we are interested
        team_best_points[team] = list(team_best_points[team])[:3]
    ...
    for team, players in all_players.items():
        ...
        get_highest = team_best_points[team]
        for player in players:
            if(len(list(player.values())) <= 2):
                continue
            points = player["points"]
            for x in range(len(get_highest)):
                if(points == get_highest[x]):
                    get_highest[x] = player 
        best[team] = get_highest
...
def plot_best(best: Dict[str, List[Dict]], stat: str = "points") -> None:
    ...
    """
    stats_dir = os.getcwd() + "/NBA_player_statistics"
    ...
    # iterate through each team and the players
    fig, ax = plt.subplots()
    for team, players in best.items():
        ...
        ax.barh(names, points, color=color_table[team], label=team)
    ax.set_xlabel(stat)
    ax.set_ylabel("Names")
    # set the title
    ax.set_title(f"{stat} per game")
    ...
    fig.tight_layout()
    plt.savefig(stats_dir + f"{stat}.png")
    # Important to clear the figure so that we don´t overwrite when calling this function "n" times
    plt.clf()
...
def get_player_stats(player_url: str, team: str) -> dict:
    ...
    soup = BeautifulSoup(html, "html.parser")
    if(soup.find(id="Regular_season") == None):
        return None
    ...
    for row in rows[1:]:
        ...
        if(cols != None):
            ...
            # Refers to the 2021-22 season
            if(year == 2021):
                # load stats from columns
                # keys should be 'points', 'assists', etc.
                relevant_colums = cols[8:]
                assist_nr = float(str(relevant_colums[1].text.strip()).replace("*", ""))
                stats["assists"] = assist_nr
                points_nr = float(str(relevant_colums[-1].text.strip()).replace("*", ""))
                stats["points"] = points_nr
                rebound_nr = float(str(relevant_colums[0].text.strip()).replace("*", ""))
                stats["rebounds"] = rebound_nr
                break
    return stats
